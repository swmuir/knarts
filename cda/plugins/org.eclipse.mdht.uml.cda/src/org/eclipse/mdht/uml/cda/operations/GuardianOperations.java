	...
	public static boolean validateGuardianChoice(Guardian guardian, DiagnosticChain diagnostics,
		...
		if (VALIDATE_GUARDIAN_CHOICE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			synchronized (EOCL_ENV) {
				...
				try {
					VALIDATE_GUARDIAN_CHOICE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
						helper.createInvariant(VALIDATE_GUARDIAN_CHOICE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
				} catch (ParserException pe) {
					throw new UnsupportedOperationException(pe.getLocalizedMessage());
				}
			}
		}
		if (!EOCL_ENV.get().createQuery(VALIDATE_GUARDIAN_CHOICE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
			guardian)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
